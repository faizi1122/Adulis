...
service = ReportPortalService(endpoint=rp_endpoint, project=rp_project,
                              token=rp_token)
...
def start_test(name, start_time, item_type, description=None, tags=None):
    ...
    """
    item_id = service.start_test_item(name=name, description=description, tags=tags, start_time=start_time, item_type=item_type)
    return item_id


def finish_test(end_time, status, issue=None, item_id=None):
    service.finish_test_item(item_id=item_id, end_time=end_time, status=status, issue=issue)
...
def finish_feature(end_time, status, issue=None, item_id=None):
    """
